    ...
    <div className="min-h-screen bg-transparent">
      ...
      <section className="pt-28 pb-16 px-6 text-center">
        <div className="max-w-3xl mx-auto">
          ...
          <h1 className="font-display text-5xl font-black text-white mb-5 leading-tight">
            Precios claros,<br />
          ...
          <div className="flex items-center justify-center gap-4 mt-10">
            <span className={`text-sm font-semibold transition ${!anual ? "text-white" : "text-gray-500"}`}>Mensual</span>
            <button
              onClick={() => setAnual(a => !a)}
              className={`relative w-12 h-6 rounded-full transition-colors ${anual ? "bg-purple-600" : "bg-white/15"}`}
            >
              <span
            ...
            <span className={`text-sm font-semibold transition ${anual ? "text-white" : "text-gray-500"}`}>
              Anual
          ...
          {PLANES.map((plan) => (
            <div
              ...
              className={`relative rounded-2xl border p-8 flex flex-col transition-all ${
                plan.highlight
                  ? "border-cyan-500/50 bg-[#0d1a2b] shadow-[0_0_40px_rgba(0,212,255,0.12)]"
                  : "border-white/10 bg-[#0d0d2b]/60"
              }`}
            >
              {plan.highlight && (
                <div className="absolute -top-3.5 left-1/2 -translate-x-1/2">
                  <span className="bg-gradient-to-r from-cyan-500 to-purple-500 text-white text-xs font-bold px-4 py-1.5 rounded-full">
                    Más popular
                  </span>
                </div>
              )}
              ...
              <div className="mb-6">
                ...
                <div className="flex items-end gap-1 mb-3">
                  ...
                  <span className="text-gray-500 text-sm pb-2">/mes</span>
                </div>
                {anual && (
              ...
              <div className="space-y-3">
                ...
                <p className="text-center text-xs text-gray-600">{plan.ideal}</p>
              </div>
            </div>
          ))}
        </div>
            ...
            {FAQS.map((faq, i) => (
              ...
              >
                <button
                  ...
                  className="w-full flex items-center justify-between px-5 py-4 text-left text-sm font-semibold text-white hover:text-purple-300 transition"
                >
                  {faq.q}
                  <span className={`text-lg text-gray-500 transition-transform ${openFaq === i ? "rotate-45" : ""}`}>+</span>
                </button>
                {openFaq === i && (
      ...
      <section className="py-20 px-6 border-t border-purple-500/15">
        <div className="max-w-xl mx-auto text-center">
          ...
          <p className="mt-4 text-xs text-gray-600">Respuesta en menos de 2 horas · Sin tarjeta</p>
        </div>
      </section>
