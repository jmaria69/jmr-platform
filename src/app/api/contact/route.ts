...
export async function POST(request: NextRequest) {
    try {
        ...
        await resend.emails.send({
            ...
            html: `<h2>Hola ${data.nombre},</h2><p>Gracias por contactarnos. Nos pondremos en contacto pronto.</p><p><strong>Proyecto:</strong> ${data.proyecto}</p>`,
        });
        ...
        await resend.emails.send({
            ...
            html: `<h3>Nuevo contacto</h3><p><strong>Nombre:</strong> ${data.nombre}</p><p><strong>Email:</strong> ${data.email}</p><p><strong>Empresa:</strong> ${data.empresa || 'N/A'}</p><p><strong>Proyecto:</strong> ${data.proyecto}</p><p><strong>Mensaje:</strong> ${data.mensaje.replace(/\n/g, '<br>')}</p>`,
        });
